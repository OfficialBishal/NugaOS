...
InterruptManager::InterruptManager(uint16_t hardwareInterruptOffset, GlobalDescriptorTable* globalDescriptorTable, TaskManager* taskManager)
    : programmableInterruptControllerMasterCommandPort(0x20),   //instantiate port_no for interrupt
      ...
      programmableInterruptControllerSlaveDataPort(0xA1)
{
    ...
    uint32_t CodeSegment = globalDescriptorTable->CodeSegmentSelector();    // get offset of CodeSegment
    ...
    // remap
    programmableInterruptControllerMasterDataPort.Write(hardwareInterruptOffset);   // telling to add 0x20 to interrupt since 0x01 sent ky keyboard is used for exception by CPU too.
    programmableInterruptControllerSlaveDataPort.Write(hardwareInterruptOffset+8);

    programmableInterruptControllerMasterDataPort.Write(0x04);  // tell master, it is master
    programmableInterruptControllerSlaveDataPort.Write(0x02);   // tell slave, it is slave
    ...
    asm volatile("lidt %0" : : "m" (idt_pointer));      //load the IDT
}
